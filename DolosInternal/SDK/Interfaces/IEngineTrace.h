...
inline bool Trace(IEngineTrace* pEngineTrace, IClientEntity* pLocalPlayer, IClientEntity* pEntity, Vector vFrom, Vector vTo)
{
    CGameTrace trace;
    Ray_t ray;
    ...
    pEngineTrace->TraceRay(ray, MASK_SHOT | CONTENTS_GRATE, &tracefilter, &trace);
    if (trace.hit_entity == pEntity)
    {
        return true;
    }
    return false;
